...
public class FollowTask extends Task {
	...
	public FollowTask(Follow followData) {
		this.follow = followData;
	}

	@Override
	public void run() {
		Log.log("Starting follow task for " + follow.getUser().getAccount().getUser() + " will follow " + follow.getSize()
		...
		do {
			if (this.intervalPassed(follow.getInterval())) {
				try {
					...
					Log.log(follow.getUser().getAccount().getUser() + "  failed to follow" + e.getMessage());
				}
				followList.remove(0);
